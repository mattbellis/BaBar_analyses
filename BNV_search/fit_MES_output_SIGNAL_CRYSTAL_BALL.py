...
def main(argv):
    ...
    ############################################################################
    # Create a Crystal Barrel function for signal and its
    # their parameters
    pars_sig, cb_sig = crystal_barrel_x(x,'sig')
    pars_sig[0].setVal(5.279)
    #pars_sig[0].setRange(0.8,1.5)
    pars_sig[0].setRange(5.2,5.3)

    pars_sig[1].setVal(0.0028)
    pars_sig[1].setRange(0.002,0.004)

    pars_sig[2].setVal(2.0)
    pars_sig[2].setRange(1.5, 3.0)

    pars_sig[3].setVal(1.0)         # n, not the number of entries
    pars_sig[3].setRange(0.0, 10.0)
    ...
    # nsig*sig+nbkg*bkg
    nsig = ROOT.RooRealVar("nsig", "number of signal events", 40000, 0., 1000000)
    ...
    model = ROOT.RooAddPdf("model_sig", "a1", ROOT.RooArgList(cb_sig), ROOT.RooArgList( nsig))
