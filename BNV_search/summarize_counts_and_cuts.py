...
for decay in decays:
    ...
    # CUTS 2
    output += 'SP & & Raw events & Previous cuts & Cuts step 2 & Cuts \% of prev & Cuts \% of raw \\\\\n'
    output += '\\hline\n'
    for sp in sps:
        ...
        # CUTS 1
        #output += '{0:5} & {1:12} & {2:10d} & {3:10d} & {4:6.2e} & {5:10d} & {6:6.2e} \\\\\n'.format( sp, sptag[1], raw, skim, 100*skim/raw, cuts1, 100*cuts1/raw)
        # CUTS 2
        if raw>0 and cuts1>0:
            output += '{0:5} & {1:12} & {2:10d} & {3:10d} & {4:10d} & {5:6.2e} & {6:6.2e} \\\\\n'.format( sp, sptag[1], raw, cuts1, cuts2,  100*cuts2/cuts1, 100*cuts1/raw)
...
for decay in decays:
    ...
    for run in runs:
        ...
        # CUTS 1
        #output += '{0:5} & {1:12} & {2:10d} & {3:10d} & {4:6.2e} & {5:10d} & {6:6.2e} \\\\\n'.format( run, sptag[1], raw, skim, 100*skim/raw, cuts1, 100*cuts1/raw)
        # CUTS 2
        if raw>0 and cuts1>0:
            output += '{0:5} & {1:12} & {2:10d} & {3:10d} & {4:10d} & {5:6.2e} & {6:6.2e} \\\\\n'.format( run, sptag[1], raw, cuts1, cuts2,  100*cuts2/cuts1, 100*cuts1/raw)
