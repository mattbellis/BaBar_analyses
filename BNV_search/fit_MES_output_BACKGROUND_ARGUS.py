...
def main(argv):
    ...
    # their parameters
    pars_bkg, arg_bkg = argus_in_x(x,'bkg')
    pars_bkg[0].setVal(-20)
    pars_bkg[0].setRange(-1000,1000)
    ...
    # nsig*sig+nbkg*bkg
    nbkg = ROOT.RooRealVar("nbkg", "number of background events", 400, 0., 1000000)
    model = ROOT.RooAddPdf("model_bkg", "a1", ROOT.RooArgList(arg_bkg), ROOT.RooArgList( nbkg))
    #model = arg_bkg
